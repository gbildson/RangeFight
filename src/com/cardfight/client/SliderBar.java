...
public class SliderBar extends FocusPanel implements ResizableWidget,
    ...
    public void run() {
      ...
      if (shiftRight) {
    	if (knob1HasFocus)
    		...
    		setCurrentValue2(curValue2 + multiplier * stepSize);
      } else {
      	if (knob1HasFocus)
      		setCurrentValue(curValue - multiplier * stepSize);
      	else 
  ...
  private boolean displaySecondKnob = false;
  private boolean knob1HasFocus = true;

  /**
   * The knob that slides across the line.
   */
  private Image knobImage = new Image();
  private Image knobImage2 = new Image();
  ...
  public void setCurrentValue2(double curValue2) {
	  setCurrentValue2(curValue2, true);
  }
  
  public void setCurrentValue2(double curValue2, boolean fireEvent) {
	this.curValue2 = curValue2;
	if ( !displaySecondKnob ) {
	    //Knob2
	    images.slider().applyTo(knobImage2);
	    Element knobElement2 = knobImage2.getElement();
	    DOM.appendChild(getElement(), knobElement2);
	    DOM.setStyleAttribute(knobElement2, "position", "absolute");
	    DOM.setElementProperty(knobElement2, "className", "gwt-SliderBar-knob");
	}
	displaySecondKnob = true;
	
    // Confine the value to the range
    this.curValue2 = Math.max(minValue, Math.min(maxValue, curValue2));
    double remainder = (this.curValue2 - minValue) % stepSize;
    this.curValue2 -= remainder;

    // Go to next step if more than halfway there
    if ((remainder > (stepSize / 2)) && ((this.curValue2 + stepSize) <= maxValue)) {
      this.curValue2 += stepSize;
    }
	
	setCurrentValue(curValue, fireEvent);
  }
  ...
  public void shiftLeft(int numSteps) {
	  if ( knob1HasFocus )
		  ...
		  setCurrentValue2(getCurrentValue2() - numSteps * stepSize);
  }
  ...
  protected double getKnob2Percent() {
	  ...
	  // Calculate the relative progress
	  double percent = (curValue2 - minValue) / (maxValue - minValue);
	  return Math.max(0.0, Math.min(1.0, percent));
  }
  ...
  private void drawKnob() {
    ...
    if ( displaySecondKnob ) {
    Element knobElement2 = knobImage2.getElement();
    int lineWidth2 = DOM.getElementPropertyInt(lineElement, "offsetWidth");
    int knobWidth2 = DOM.getElementPropertyInt(knobElement2, "offsetWidth");
    int knobLeftOffset2 = (int) (lineLeftOffset + (getKnob2Percent() * lineWidth) - (knobWidth / 2));
    knobLeftOffset2 = Math.min(knobLeftOffset2, lineLeftOffset + lineWidth - (knobWidth / 2) - 1);
    DOM.setStyleAttribute(knobElement2, "left", knobLeftOffset2 + "px");
    }
    //
  ...
  private void startSliding(boolean highlight, boolean fireEvent) {
    if (highlight) {
      ...
      if ( knob1HasFocus )
          ...
          images.sliderSliding().applyTo(knobImage2);
    }
  }
